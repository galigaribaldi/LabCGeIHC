...
Model modelRailRoad;
Model modelAircraft;
Model modelEclipseChasis, modelWheelsF, modelWheelsR;
Model heliChasis, heli;
...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	modelWheelsF.loadModel("../models/Eclipse/2003eclipse_frontal_wheels.obj");
	modelWheelsF.setShader(&shaderMulLighting);

	modelWheelsR.loadModel("../models/Eclipse/2003eclipse_rear_wheels.obj");
	modelWheelsR.setShader(&shaderMulLighting);

	//Helicoptero
	heliChasis.loadModel("../models/Helicopter/Mi_24_chasis.obj");
	heliChasis.setShader(&shaderMulLighting);

	heli.loadModel("../models/Helicopter/Mi_24_heli.obj");
...
void applicationLoop() {
	...
	float rotCount = 0.0, rotWheelx = 0.0, rotWheely = 0.0;
	float rotHelixY = 0.0;

	lastTime = TimeManager::Instance().GetTime();
	while (psi) {
		currTime = TimeManager::Instance().GetTime(); //Recuperamos tiempo actual
		if (currTime - lastTime < (0.0167)) {
			glfwPollEvents();
			continue;
		}
		lastTime = currTime;
		...
		glm::mat4 frontWheels = glm::mat4(modelMatrixEclipseChasis);
		frontWheels = glm::translate(frontWheels, glm::vec3(0, 1.06285, 4.11795)); //y-> z y z->y
		frontWheels = glm::rotate(frontWheels, rotWheely, glm::vec3(0, 1, 0));
		frontWheels = glm::rotate(frontWheels, rotWheelx, glm::vec3(1, 0, 0));
		frontWheels = glm::translate(frontWheels, glm::vec3(0, -1.06285, -4.11795)); //y-> z y z->y
		modelWheelsF.render(frontWheels);
		glActiveTexture(GL_TEXTURE0);

		glm::mat4 rearWheels = glm::mat4(modelMatrixEclipseChasis);
		rearWheels = glm::translate(rearWheels, glm::vec3(0, 1.05128, -4.34651)); //y-> z y z->y (se obtienen de blender)
		rearWheels = glm::rotate(rearWheels, rotWheelx, glm::vec3(1, 0, 0));
		rearWheels = glm::translate(rearWheels, glm::vec3(0, -1.05128, 4.34651)); //y-> z y z->y
		modelWheelsR.render(rearWheels);
		glActiveTexture(GL_TEXTURE0);

		//Render Helicoptero
		glm::mat4 copterBody = glm::mat4(1.0);
		copterBody = glm::translate(copterBody, glm::vec3(-10, 5.0, 5.0));
		heliChasis.render(copterBody);
		glActiveTexture(GL_TEXTURE0);

		/*
		 * Cords Blender
		 * 0.003344,-1.88318,0.254566
		 * OpenGL
		 *
		 * Pivote
		 * 0.003344,-1.88318,0.254566
		...
		*/
		...
		glm::mat4 copterHelix = glm::mat4(copterBody);
		copterHelix = glm::translate(copterHelix, glm::vec3(-0.003344, 1.88318, -0.254566)); //Pivote Invertido
		copterHelix = glm::rotate(copterHelix, rotHelixY, glm::vec3(0, 1, 0));
		copterHelix = glm::translate(copterHelix, glm::vec3(0.003344, -1.88318, 0.254566));
		heli.render(copterHelix);
		glActiveTexture(GL_TEXTURE0);
